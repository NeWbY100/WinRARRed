...
public partial class SettingsOptionsForm : Form
{
    ...
    public string? ExtractedVerificationFilePath { get; private set; }
    ...
    private CustomPackerType ImportedCustomPackerType = CustomPackerType.None;
    private string? ImportedSrrFilePath = null;
    ...
    private void OptionsForm_Shown(object? sender, EventArgs e)
    {
        ...
        cbFileA.CheckState = (CheckState)RAROptions.SetFileArchiveAttribute;
        cbFileI.CheckState = (CheckState)RAROptions.SetFileNotContentIndexedAttribute;
    ...
    private void BtnImportSrr_Click(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (srr.HasCustomPackerHeaders)
            {
                string packerWarning = srr.CustomPackerDetected switch
                {
                    CustomPackerType.AllOnesWithLargeFlag =>
                        ...
                        "Direct SRR reconstruction will be used instead of brute-forcing.",
                ...
                importedSettings.Add($"⚠ CUSTOM PACKER DETECTED ({srr.CustomPackerDetected}) — will use direct reconstruction");

                ImportedCustomPackerType = srr.CustomPackerDetected;
                ImportedSrrFilePath = ofd.FileName;

                if (!GUIHelper.ShowWarning(this, packerWarning))
                {
                    Log.Information(this, "SRR import cancelled by user", LogTarget.System);
                    return;
                }
            }
            else
            {
                ImportedCustomPackerType = CustomPackerType.None;
                ImportedSrrFilePath = null;
            }
            ...
            {
                ExtractedVerificationFilePath = extractedVerificationFilePath;
            ...
            // Auto-close when custom packer detected — MainForm will open reconstruction form
            if (ImportedCustomPackerType != CustomPackerType.None)
            {
                RAROptions = GetRAROptions();
                DialogResult = DialogResult.OK;
                return;
    ...
    private RAROptions GetRAROptions()
    {
        ...
        return new()
        {
            SetFileArchiveAttribute = (TriState)cbFileA.CheckState,
            SetFileNotContentIndexedAttribute = (TriState)cbFileI.CheckState,
            ...
            // Custom packer direct reconstruction
            CustomPackerDetected = ImportedCustomPackerType,
            SrrFilePath = ImportedSrrFilePath
        };
    }
