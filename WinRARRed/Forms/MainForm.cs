...
public partial class MainForm : Form
{
    ...
    private void TsmiSettingsOptions_Click(object? sender, EventArgs e)
    {
            ...
            && !string.IsNullOrEmpty(OptionsForm.RAROptions.SrrFilePath))
        {
            using var form = new SRRReconstructionForm(
                OptionsForm.RAROptions.SrrFilePath,
                tbReleaseDirectory.Text,
                tbOutputDirectory.Text,
                OptionsForm.ExtractedVerificationFilePath ?? tbVerificationFilePath.Text);
    ...
    {
        // Route to progress form
        progressForm?.UpdateOverallProgress(e);
    ...
    {
        progressForm?.UpdateSubProgress(e);
    }
    ...
    private async void BtnStart_Click(object? sender, EventArgs e)
    {
        ...
        // Show progress form
        progressForm = new BruteForceProgressForm();
        progressForm.StopRequested += (s, e) =>
        {
            Manager?.Stop();
            btnStart.Enabled = false;
        };
        ...
        Manager = new(new SerilogReSceneLogger());
        ...
        try
        {
            ...
            StopElapsedTimer();
            progressForm.SetCompleted(success);
    ...
    {
        foreach (Control control in tlpInput.Controls)
        {
            if (control != btnStart)
            {
